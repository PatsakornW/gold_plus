    ...
    <div class="col-12 col-md-10 col-lg-10 col-xl-10">
        <div class="card rounded-4 ">
            <div class="card-header" style="background-color: #B0120A;">
                <p class="text-center text-white fs-5  m-0 p-0  fw-bold">รายการแลกทอง</p>
            </div>
            <div class="card-body">
                 ...
                 <div class="table-responsive">
                    <table class="table table-bordered ">
                        ...
                        <tbody>
                            @php($counter = ($data_all_trade_golds->currentPage() - 1) * $data_all_trade_golds->perPage())
                            @foreach ($data_all_trade_golds as $value)
                                <tr>
                                    <td>{{ ++$counter }}</td>
                                    <td>{{ $value->created_at }}</td>
                                    <td>{{ $value->name }}</td>
                                    <td class="text-center">{{ $value->total }}</td>
                                    <td class="text-warning fw-bold text-center">{{ $value->type }}</td>
                                    <td class="text-center">
                                        <button type="button" class="border-0 border bg-white"
                                            data-bs-toggle="modal"
                                            data-bs-target="#detail_list_tradegold" onclick="showModal('{{ json_encode($value) }}')"><i
...
<!-- modal   -->
<div class="modal fade" id="detail_list_tradegold" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
