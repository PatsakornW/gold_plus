            ...
            <div class="card-body">
                 ...
                 <div class="table-responsive">
                    <table class="table table-bordered ">
                        ...
                        <tbody>
                            @php($counter = ($data_all_withdraws->currentPage() - 1) * $data_all_withdraws->perPage())
                            @foreach ($data_all_withdraws as $value)
                                <tr>
                                    <td>{{ ++$counter }}</td>
                                    <td>{{ $value->created_at }}</td>
                                    <td>{{ $value->name }}</td>
                                    <td class="text-center">{{ $value->total }}</td>
                                    <td class="text-danger fw-bold text-center">{{ $value->type }}</td>
                                    <td class="text-center">
                                        <button type="button" class="border-0 border bg-white"
                                            data-bs-toggle="modal"
                                            data-bs-target="#detail_list_withdraw" onclick="showModal('{{ json_encode($value) }}')"><i
...
<!-- modal   -->
<div class="modal fade" id="detail_list_withdraw" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
